...
configuro:
    ...
    movlb 4H
    movlw 0F0H
    movwf TRISD, 1 ;((PORTD) and 0FFh), 0, a al ((PORTD) and 0FFh), 3, a como salidas
    movwf ANSELD, 1 ;((PORTD) and 0FFh), 0, a al ((PORTD) and 0FFh), 3, a como digitales
    ...
    movlw 0FCH
    movwf ANSELF, 1 ;((PORTF) and 0FFh), 1, a y ((PORTF) and 0FFh), 0, a como digitales
    movlw 03H
    movwf WPUF, 1 ;((PORTF) and 0FFh), 1, a y ((PORTF) and 0FFh), 0, a con pullup actividades

inicio:
    btfsc PORTF, 0, 1 ;Pregunto si ((PORTF) and 0FFh), 0, a es cero (si presione el boton1)
    bra preguntaRF1 ;falso, salta a etiqueta pregunta ((PORTF) and 0FFh), 1, a
    btg LATB, 0, 1 ;verdad, aplico complemento a ((PORTB) and 0FFh), 0, a
otro1:
    btfss PORTF, 0, 1 ;Pregunto si ((PORTF) and 0FFh), 0, a es uno (si solte el boton1)
    bra otro1 ;falso, regresa a pregunta si ((PORTF) and 0FFh), 0, a=1
    bra inicio ;verdad, regresa a inicio
preguntaRF1:
    btfsc PORTF, 1, 1 ;Pregunto si ((PORTF) and 0FFh), 1, a es cero (si presione el boton2)
