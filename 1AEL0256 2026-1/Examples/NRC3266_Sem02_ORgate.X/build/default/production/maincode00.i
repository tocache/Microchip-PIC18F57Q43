...
configuro:
    ...
    movlw 0FCH
    movwf ANSELE, 1 ;((PORTE) and 0FFh), 1, a y ((PORTE) and 0FFh), 0, a como digitales
    bcf TRISF, 0, 1 ;((PORTF) and 0FFh), 0, a como salida
    bcf ANSELF, 0, 1 ;((PORTF) and 0FFh), 0, a como digital

inicio:
    btfsc PORTE, 0, 1 ;Pregunto si ((PORTE) and 0FFh), 0, a es cero
    bra es_falso ;falso, salta a label es_falso
    btfsc PORTE, 1, 1 ;verdad y pregunta si ((PORTE) and 0FFh), 1, a es cero
    bra es_falso ;falso y salta a label es_falso
    bcf LATF, 0, 1 ;verdad y ((PORTF) and 0FFh), 0, a=0 (LED off)
    bra inicio ;salto a label inicio
es_falso:
    bsf LATF, 0, 1 ;((PORTF) and 0FFh), 0, a=1 (LED on)
    bra inicio ;salto a label inicio
