...
configuro:
    ...
    bcf ANSELE, 0, 1 ;((PORTE) and 0FFh), 0, a es digital
    bcf TRISB, 0, 1 ;((PORTB) and 0FFh), 0, a es salida
    bcf TRISB, 1, 1 ;((PORTB) and 0FFh), 1, a es salida
    ;Recordando bcf [registro], #bit, access bank o BSR (1 o 0)
    bcf ANSELB, 0, 1 ;((PORTB) and 0FFh), 0, a es digital
    bcf ANSELB, 1, 1 ;((PORTB) and 0FFh), 1, a es digital

inicio_tren:
    btfss PORTE, 0, 1 ;pregunto si ((PORTE) and 0FFh), 0, a es 1
    bra es_falsazo ;viene aqui cuando es Falso
    bra es_verdad ;viene aqui cuando es Verdadero
es_verdad:
    bsf LATB, 0, 1 ;enciendo ((PORTB) and 0FFh), 0, a
    bcf LATB, 1, 1 ;apago ((PORTB) and 0FFh), 1, a
    call retardo ;llamada a subrutina retardo
    bcf LATB, 0, 1 ;apago ((PORTB) and 0FFh), 0, a
    bsf LATB, 1, 1 ;enciendo ((PORTB) and 0FFh), 1, a
    call retardo ;llamada a subrutina retardo
