...
# 1 "<built-in>" 2
# 1 "maincode.s" 2
;Ejemplo de latch para LED en RF3 empleando INT0 conectado a RB4
    PROCESSOR 18F57Q43
# 1 "./cabecera.inc" 1
...
INT0_ISR:
    bcf PIR1, 0, 1 ;bajamos bandera ((PIR1) and 0FFh), 0, a
    btg LATF, 3, 1 ;basculamos LED que esta en ((PORTF) and 0FFh), 3, a
    retfie

    PSECT INT1_ISR, class=CODE, reloc=4
INT1_ISR:
    bcf PIR6, 0, 1 ;bajamos bandera ((PIR1) and 0FFh), 0, a
    btg LATE, 0, 1 ;basculamos LED que esta en ((PORTE) and 0FFh), 0, a
    retfie

    PSECT INT2_ISR, class=CODE, reloc=4
INT2_ISR:
    bcf PIR10, 0, 1 ;bajamos bandera ((PIR1) and 0FFh), 0, a
    btg LATC, 0, 1 ;basculamos LED que esta en ((PORTC) and 0FFh), 0, a
    retfie
...
configuro:
    ...
    movlw 0CH
    movwf INT0PPS, 1 ;asignación de INT0 a ((PORTB) and 0FFh), 4, a
    movlw 1BH
    movwf INT1PPS, 1 ;asignación de INT1 a ((PORTD) and 0FFh), 3, a
    movlw 29H
    movwf INT2PPS, 1 ;asignación de INT2 a ((PORTF) and 0FFh), 1, a
    movlb 4H
    bcf TRISF, 3, 1
    bcf ANSELF, 3, 1
    bcf TRISE, 0, 1
    ...
    clrf IVTBASEH, 1
    clrf IVTBASEL, 1
    bsf PIE1, 0, 1 ;activando INT0 con ((PIE1) and 0FFh), 0, a=1
    bcf PIR1, 0, 1 ;bajo bandera ((PIR1) and 0FFh), 0, a
    bsf PIE6, 0, 1 ;activando INT1 con ((PIE6) and 0FFh), 0, a=1
    bcf PIR6, 0, 1 ;bajo bandera ((PIR6) and 0FFh), 0, a
    bsf PIE10, 0, 1 ;acitvando INT2 con ((PIE10) and 0FFh), 0, a=1
    bcf PIR10, 0, 1 ;bajo bandera ((PIR10) and 0FFh), 0, a
    bcf INTCON0, 0, 1 ;((INTCON0) and 0FFh), 0, a=0 para detección de flanco negativo
    bcf INTCON0, 1, 1 ;((INTCON0) and 0FFh), 1, a=0 para deteccion de flanco negativo
    bcf INTCON0, 2, 1 ;((INTCON0) and 0FFh), 2, a=0 para deteccion de flanco negativo
    bsf INTCON0, 7, 1 ;((INTCON0) and 0FFh), 7, a=1 habilitando el interruptor global

inicio: ;Rutina principal no hace nada
    bra inicio
