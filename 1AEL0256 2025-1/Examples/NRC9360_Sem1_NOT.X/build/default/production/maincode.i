...
configuro:
    ...
    ;Para ((PORTB) and 0FFh), 2, a:
    bsf TRISB, 2, 1 ;((PORTB) and 0FFh), 2, a como entrada
    bcf ANSELB, 2, 1 ;((PORTB) and 0FFh), 2, a como digital
    ;Para ((PORTC) and 0FFh), 1, a:
    bcf TRISC, 1, 1 ;((PORTC) and 0FFh), 1, a como salida
    bcf ANSELC, 1, 1 ;((PORTC) and 0FFh), 1, a como digital

inicio:
    btfss PORTB, 2, 1 ;Pregunto si ((PORTB) and 0FFh), 2, a es uno, si presione el boton
    bra noescierto ;Falso, salto a label noescierto
    bra siescierto ;Verdad, salto a label siescierto

noescierto:
    bsf LATC, 1, 1 ;((PORTC) and 0FFh), 1, a=1, enciendo el LED
    bra inicio ;retorno a label inicio

siescierto:
    bcf LATC, 1, 1 ;((PORTC) and 0FFh), 1, a=0, apago el LED
