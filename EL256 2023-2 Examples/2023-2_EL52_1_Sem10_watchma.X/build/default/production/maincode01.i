...
void lcd_init(void){
    ...
    ANSELD = 0x00;
    LCD_CONFIG();
    _delay((unsigned long)((23)*(32000000UL/4000.0)));
    BORRAR_LCD();
    CURSOR_HOME();
    CURSOR_ONOFF(1);
}
...
void main(void) {
    ...
    lcd_init();
    POS_CURSOR(1,2);
    ESCRIBE_MENSAJE("Relojazo UPC",12);
    while(1){
        POS_CURSOR(2,2);
        convierte(horas);
...
void __attribute__((picinterrupt(("irq(30)")))) CCP1_ISR(void){
    PIR3bits.CCP1IF = 0;
    if(centesimas == 99){
        centesimas = 0;
        if(segundos == 59){
...
void __attribute__((picinterrupt(("irq(8)")))) INT0_ISR(void){
    PIR1bits.INT0IF = 0;
    if(horas == 23){
        horas = 0;
    }
...
void __attribute__((picinterrupt(("irq(48)")))) INT1_ISR(void){
    PIR6bits.INT1IF = 0;
    if(minutos == 59){
        minutos = 0;
    }
