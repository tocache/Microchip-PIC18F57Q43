...
configuro:
    ...
    bsf TRISB, 4, 1 ;puerto ((PORTB) and 0FFh), 4, a sea entrada
    bcf ANSELB, 4, 1 ;puerto ((PORTB) and 0FFh), 4, a como digital
    bsf WPUB, 4, 1 ;puerto ((PORTB) and 0FFh), 4, a con
    bcf TRISF, 3, 1 ;puerto ((PORTF) and 0FFh), 3, a como salida
    bcf ANSELF, 3, 1 ;puerto ((PORTF) and 0FFh), 3, a como digital

inicio: ;Programa de la aplicacion
    btfsc PORTB, 4, 1 ;Pregunto si ((PORTB) and 0FFh), 4, a es cero (pulse el boton)
    bra nopulsaste ;Salta aqui cuando es falso
    bra sipulsaste ;Salta aca cuando es verdadero

nopulsaste:
    bcf LATF, 3, 1 ;((PORTF) and 0FFh), 3, a a cero (se enciende el LED)
    bra inicio ;Salta a inicio

sipulsaste:
    bsf LATF, 3, 1 ;((PORTF) and 0FFh), 3, a a uno (se apaga el LED)
    bra inicio ;Salta a inicio
