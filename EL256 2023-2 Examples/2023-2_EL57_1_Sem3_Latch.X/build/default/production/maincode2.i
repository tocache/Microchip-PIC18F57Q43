...
configuro:
    ...
    bsf WPUB, 4, 1
    movlw 0FCH
    movwf TRISD, 1 ;((PORTD) and 0FFh), 0, a y ((PORTD) and 0FFh), 1, a como salidas
    movwf ANSELD, 1 ;((PORTD) and 0FFh), 0, a y ((PORTD) and 0FFh), 1, a como digitales
    movlw 01H
    movwf LATD, 1 ;((PORTD) and 0FFh), 0, a encendido y ((PORTD) and 0FFh), 1, a apagado

inicio:
    btfsc PORTB, 4, 1 ;Pregunta si ((PORTB) and 0FFh), 4, a es cero (si presione el boton)
    bra inicio ;falso, regresa a preguntar
    comf LATD, 1, 1 ;complemento a registro
    call retardon
otro:
    btfss PORTB, 4, 1 ;Pregunta si ((PORTB) and 0FFh), 4, a es uno (si solte el boton)
