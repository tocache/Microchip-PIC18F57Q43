...
configuro:
    ...
    ;((PORTF) and 0FFh), 4, a entrada digital con pullup activado
    bsf TRISF, 4, 1 ;((PORTF) and 0FFh), 4, a es entrada
    bcf ANSELF, 4, 1 ;((PORTF) and 0FFh), 4, a es digital
    bsf WPUF, 4, 1 ;((PORTF) and 0FFh), 4, a con pullup activo
    ;((PORTD) and 0FFh), 3, a salida digital
    bcf TRISD, 3, 1 ;((PORTD) and 0FFh), 3, a es salida
    bcf ANSELD, 3, 1 ;((PORTD) and 0FFh), 3, a es digital

;rutina de la aplicación:
inicio:
    btfsc PORTF, 4, 1 ;preguntamos si ((PORTF) and 0FFh), 4, a es cero (si has pulsado el boton)
    bra no_presionaste ;salto a label no_presionaste
    bra si_presionaste ;salto a label si_presionaste

no_presionaste:
    ;mantener el LED apagado
    bcf LATD, 3, 1 ;((PORTD) and 0FFh), 3, a en bajo (LED apagado)
    bra inicio ;Salta a label inicio

si_presionaste:
    ;parpadear el LED
    bsf LATD, 3, 1 ;((PORTD) and 0FFh), 3, a en alto (LED encendido)
