...
configuro: ;Zona de conf de la app
    ...
    ;((PORTB) and 0FFh), 4, a sea entrada digital con pullup activado
    bsf TRISB, 4 ;((TRISB) and 0FFh), 4, a debe de ser uno para que sea entrada
    bcf ANSELB, 4 ;((ANSELB) and 0FFh), 4, b debe de ser cero para que sea digital
    bsf WPUB, 4 ;((WPUB) and 0FFh), 4, b debe de ser uno para que se active la pullup
    ;((PORTF) and 0FFh), 3, a sea salida digital
    bcf TRISF, 3 ;((TRISF) and 0FFh), 3, a debe de ser cero para que sea salida
    bcf ANSELF, 3 ;((ANSELF) and 0FFh), 3, b debe de ser cero para que sea digital

inicio:
    ;Leo el ((PORTB) and 0FFh), 4, a
    btfsc PORTB, 4 ;Pregunto si ((PORTB) and 0FFh), 4, a=0
    bra falso ;Viene aqui si es falso, salta a label falso
    bsf LATF, 3 ;Viene aqui si es cierto, ((PORTF) and 0FFh), 3, a=1
    bra inicio ;Salta a label inicio
falso:
    bcf LATF, 3 ;((PORTF) and 0FFh), 3, a=0
    bra inicio ;Salta a label inicio
