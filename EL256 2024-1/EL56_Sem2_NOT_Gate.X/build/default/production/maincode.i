...
configuro:
    ...
    bsf OSCEN, 6, 1 ;((OSCEN) and 0FFh), 6, b = 1 (HFINTOSC encendido)
    movlb 04H ;nos vamos al BANK4
    ;Configuracion de las E/S: ((PORTE) and 0FFh), 0, a digital input, ((PORTF) and 0FFh), 0, a digital output
    bsf TRISE, 0, 1 ;((PORTE) and 0FFh), 0, a es entrada
    bcf ANSELE, 0, 1 ;((PORTE) and 0FFh), 0, a como digital
    bcf TRISF, 0, 1 ;((PORTF) and 0FFh), 0, a es salida
    bcf ANSELF, 0, 1 ;((PORTF) and 0FFh), 0, a como digital

inicio:
    btfsc PORTE, 0, 1 ;Pregunto si ((PORTE) and 0FFh), 0, a es uno
    bra falso ;Cuando es falso
    bra verdad ;Cuando es verdad

falso: ;Si es falso ((PORTF) and 0FFh), 0, a es uno
    bcf LATF, 0, 1 ;((PORTF) and 0FFh), 0, a=1
    bra inicio ;Regreso al inicio

verdad: ;Si es cierto ((PORTF) and 0FFh), 0, a es cero
    bsf LATF, 0, 1 ;((PORTF) and 0FFh), 0, a=0
    bra inicio ;Regreso al inicio
