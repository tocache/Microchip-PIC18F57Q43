...
configuro:
    ...
    movwf OSCEN
    movlb 4H
    bcf TRISA, 0 ;((PORTA) and 0FFh), 0, a salida
    bcf ANSELA, 0 ;((PORTA) and 0FFh), 0, a digital
    movlw 0FCH
    movwf TRISD ;((PORTD) and 0FFh), 1, a a ((PORTD) and 0FFh), 0, a como salidas
    movwf ANSELD ;((PORTD) and 0FFh), 1, a a ((PORTD) and 0FFh), 0, a como digitales
    bsf TRISB, 0 ;((PORTB) and 0FFh), 0, a como entrada
    bcf ANSELB, 0 ;((PORTB) and 0FFh), 0, a como digital
    bsf WPUB, 0 ;((PORTB) and 0FFh), 0, a con pullup
    bsf INTCON0, 7 ;((INTCON0) and 0FFh), 7, a=1
    bcf INTCON0, 0 ;((INTCON0) and 0FFh), 0, a=0
    bsf PIE1, 0 ;((PIE1) and 0FFh), 0, a=1

inicio:
    bsf LATA, 0 ;((PORTA) and 0FFh), 0, a en uno
    call repeticua ;subrutina de retardo
    bcf LATA, 0 ;((PORTA) and 0FFh), 0, a en cero
    call repeticua ;subrutina de retardo
    bra inicio

INT0_ISR:
    bcf PIR1, 0 ;Bajamos la bandera ((PIR1) and 0FFh), 0, a
    incf LATD, f
    retfie
