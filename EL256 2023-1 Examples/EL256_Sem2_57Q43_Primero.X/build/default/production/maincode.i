...
configuro: ;etiqueta configuro
    ...
    bsf WPUB, 4 ;((PORTB) and 0FFh), 4, a activado su pull-up
    bcf TRISF, 3 ;((PORTF) and 0FFh), 3, a como salida
    bcf ANSELF, 3 ;((PORTF) and 0FFh), 3, a como digital

loop: ;etiqueta loop
    ;programa de usuario
    btfss PORTB, 4 ;pregunto si ((PORTA) and 0FFh), 0, a es uno
    goto nah ;F a la preg anterior, salta a etiqueta nah
    bcf LATF, 3 ;V a la preg anterior, ((PORTC) and 0FFh), 0, a a cero
    goto loop ;salto a etiqueta loop
nah:
    bsf LATF, 3 ;((PORTC) and 0FFh), 0, a a uno
    goto loop ;salto a etiqueta loop
