...
unsigned char mensaje[]={"INGENIERIA ELECTRONICA UPAO"};
...
void decodificacion(void){
    ...
    unsigned char y_var;
    unsigned char z_var;
    for(y_var=0;y_var<8;y_var++){
        decodificado[y_var] = 0x00;
    }
    cantidad = strlen(mensaje);
    for(x_var=0;x_var<cantidad;x_var++){
        if(mensaje[x_var] == 0x20){
            decodificado[x_var+8] = 0x00;
        }
        else{
            decodificado[x_var+8] = decod[mensaje[x_var]-0x41];
        }
    }
    for(z_var=0;z_var<7;z_var++){
        decodificado[z_var+cantidad+8] = 0x00;
    }
}
...
void multiplexacion(void){
    ...
    _delay((unsigned long)((1)*(8000000UL/4000.0)));
    LATBbits.LATB5 = 0;
    LATD = decodificado[indice+6];
    LATBbits.LATB6 = 1;
    _delay((unsigned long)((1)*(8000000UL/4000.0)));
    LATBbits.LATB6 = 0;
    LATD = decodificado[indice+7];
    LATBbits.LATB7 = 1;
    _delay((unsigned long)((1)*(8000000UL/4000.0)));
    LATBbits.LATB7 = 0;
}
...
void __attribute__((picinterrupt(("irq(TMR0)")))) TMR0_ISR(void){
    PIR3bits.TMR0IF = 0;
    if(indice == cantidad+7){
        indice = 0;
    }
    else{
        indice++;
